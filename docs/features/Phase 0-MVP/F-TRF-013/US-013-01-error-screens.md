# US-013-01 — Understand an error and know what to do

**Feature:** F-TRF-013 — Errors, 404s & Degraded States | **Status:** pending

---

**Story:** As any user who hit an error, I want to see one line that explains the situation and one button that tells me what to do — plus a reference I can quote to support — so that the error explains itself instead of trapping me.
...
- Something failed: an unhandled API error (500), an unknown route (404), a rate limit, or a known error code mapped to a dedicated screen.
...
- **Rate limit (60 s):** "A moment of calm, then try again." — no Ref id (expected behavior, not a bug).
- **Known-code screens:** `TRANSFER_NOT_FOUND` / `TRANSFER_EXPIRED` / `TRANSFER_DELETED` → F-TRF-003 states; `FILES_GONE` → "Files were deleted — upload again."; `MAX_DOWNLOADS_REACHED` → "All downloads have been used." (US-003-05).
- **404 on an unknown route:** the SPA 404 screen "Oops — this page moved." + **Go to home** (FR-013-5) — distinct from an unknown *transfer* link, which stays indistinguishable from expired (F-TRF-003-9).
...
- Layout is the *only* error layout (UI-Reference §4.6): full page, centered; icon `alert`; one line `--fs-h2`; one primary button; ref line `--fs-tiny` `--fg-muted`.
- Known-code → copy map is exhaustive for MVP codes (TA-4.1.3); anything unmapped is a bug found in review (the 500 screen is the fallback, not a style choice).
- Limit screens (`TRANSFER_SIZE_EXCEEDED`, `STORAGE_QUOTA_EXCEEDED`) are *inline* on their screens (F-TRF-007), not full error pages.

## Technical notes

- Wire shape: Problem+JSON (TA-4.1.3) with the closed code list; correlation id stamped by the pipeline (T-008); React fetch wrapper (TA-8.2 `core/api`) maps `code` → screen/state.
- Telemetry: `api.requests{status}` metric (TA-10.2); 5xx spike alert (TA-10.3) — the Ref id exists precisely so the alert triage is a copy-paste.
