# US-014-02 — Have the right language automatically

**Feature:** F-TRF-014 — i18n | **Status:** pending

---

**Story:** As a user, I want the product to just know my language — no setup, no flag picker on the landing — and, if I'm signed in, to respect the language I deliberately chose in my profile.
...
- The app is loading for the first time in a browser (guest) or a signed-in user with a stored account locale.
...
- **Browser language changes mid-session:** guests are re-detected on the next load (no live switching in MVP — documented); signed-in users are unaffected (account setting stable).
- **`localStorage` cleared for a signed-in user:** the account setting re-hydrates from `/auth/me` — detection order still ends at the stored value, so no language flip.
- **The `en` default** is also the fallback language for missing keys (US-014-01) — detection and fallback share one source of truth.
...
- No visible "language detected as…" toast (calm product; no surprise UI).
- The profile `Select language` control is the only explicit control (UI-Reference §5.5).

## Technical notes

- Order implemented in `core/i18n` (TA-8.5): `localStorage` account-locale override → `navigator.language` → `en`.
- Guest override persistence: if a guest later changes nothing, nothing is stored — browser detection stays live per load (documented).
- Telemetry: locale in the `app.initialized` properties (helps the language-mix dashboard, F-TRF-012) — not a new event.
