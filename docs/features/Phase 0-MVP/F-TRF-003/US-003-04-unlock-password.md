# US-003-04 — Unlock a password-protected transfer

**Feature:** F-TRF-003 — Recipient Download Page | **Status:** pending

---

**Story:** As the recipient of a password-protected transfer, I want to enter the password and get in, so that only the intended reader can see the files — and so a refresh doesn't ask me again.
...
- The transfer is `Active` and `PasswordHash` is set (PBKDF2, TA-9.1).
- The recipient knows the password (it travels out-of-band with the link — the sender's problem to solve, F-TRF-002).
...
- **Wrong password:** field shakes + "Wrong password" (`role=alert`); **no rate-limit lockout in MVP** — but `password_wrong` is logged (FR-003-6, TA-10.2).
- **`sessionStorage` cleared / different tab or device:** re-prompted (EC-003-2 — acceptable, documented).
- **Expired transfer with a password:** the expired screen wins — no point in unlocking a dead transfer (US-003-05).
...
- Password state: single centered card, no file list visible (UI-Reference §5.3); input `type=password` with show/hide per UI-Reference §5.2 pattern; submit button primary, full width.
- Wrong password: shake animation is motion-safe (text "Wrong password" also present — F-TRF-016-5).

## Technical notes

- Endpoint 5: `POST /t/{linkId}/unlock` → verify PBKDF2 hash → issue 7-day JWT; subsequent endpoint 4 calls accept it.
- `WRONG_PASSWORD` Problem+JSON code (TA-4.1.3) maps to the shake state (F-TRF-013).
- Telemetry: `password_correct`, `password_wrong` (no password value, ever).
