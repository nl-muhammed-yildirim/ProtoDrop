...
| T-006 | Event backbone: TA-5.2 envelope, `IEventPublisher` + outbox row, SB adapter + in-memory test fake, dedup by eventId | TA-5, TA-0.2(9) | Integration test: publish → consume with fake; dedup on replay | pending |
| T-007 | Blob foundation: `IBlobStore`, SAS minting per TA-3.6, staging/transfers containers + lifecycle rules (dev Azurite), staging path helper | TA-3.5, TA-3.6 | Integration test: mint `cwr` SAS → upload block → read back | pending |
| T-008 | Pipeline foundation: correlation ID (W3C), Problem+JSON error mapping (closed code list), CORS, rate limits (API layer), telemetry SDK (TA-10.2 event helper) | TA-4.1, TA-10, F-TRF-012 | Integration test: 404/500 return Problem+JSON with correlationId; `upload_started`-style event emitted | pending |
