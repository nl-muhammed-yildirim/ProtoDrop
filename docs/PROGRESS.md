...
| **In progress** | T-005 **done 2026-09-01** — Limits Registry: `LimitsRecord` POCO + `ILimitsProvider` (wa.domain, BCL-only); `PlansCache` + `FlagsCache` (30 s TTL, injectable clock) and `LimitsProvider` (wa.application) — plan `LimitsJson` is the base, a matching `limits.{plan}.*` `FeatureFlag` OVERRIDES that one value; unit tests in `tests/wa.application.unit` (faked `IPlansStore`/`IFlagsStore` + fake clock, no Testcontainers): per-plan resolution from Part-2 `LimitsJson`, flag override wins (one field only), TTL honored (fresh ≤30 s not refetched, 30 s refetched). T-006 not started. |
