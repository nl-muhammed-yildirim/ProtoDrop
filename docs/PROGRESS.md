...
| Field | Value |
|---|---|
| **Current task** | T-006 (CURRENT) |
| **Milestone** | M0 — Foundation |
| **In progress** | T-006 **1/4** — Event backbone (TA-5) STARTED 2026-09-01: `EventEnvelope` POCO (TA-5.2, EXACTLY the 7 wire fields `eventId`/`eventType`/`version`/`occurredAt`/`correlationId`/`partitionKey`/`payload`; `eventId` = Service Bus `MessageId`, the dedup key per TA-5.1) + `IEventPublisher` port (TA-0.2(9): domain events MediatR-free — use cases call the publisher, outbox row + publish) — both in `wa.domain` (BCL-only). No new NuGet packages; no tests yet (per task 1/4). REMAINING: (2/4) outbox row + `SbEventPublisher` adapter in `wa.infrastructure/Events` (TA-2.1), (3/4) in-memory fake + dedup by eventId (15-min LRU), (4/4) integration test "publish → consume with fake; dedup on replay" + full §4 gate. |
